...
class ErrorRecoverySystem:
    ...
    def _recover_from_collapse(self, state: ResonanceState) -> Optional[ResonanceState]:
        ...
        if attractor is not None:
            ...
            rho = URCMTheory.calculate_rho(phase_vec)
            chi = max(np.linalg.norm(phase_vec), 1e-6)
            mu = URCMTheory.compute_mu(rho, chi)
            return replace(state, resonance_vector=phase_vec, stability_score=0.6,
                         rho_density=rho, chi_cost=chi, mu_value=mu)
        ...
        try:
            ...
            rho = URCMTheory.calculate_rho(neutral_vec)
            chi = max(np.linalg.norm(neutral_vec), 1e-6)
            mu = URCMTheory.compute_mu(rho, chi)
            return replace(state, resonance_vector=neutral_vec, stability_score=0.5,
                         rho_density=rho, chi_cost=chi, mu_value=mu)
        except Exception as e:
            logger.warning("Collapse recovery failed: %s", e)
            return None
